(function () {
  ...
  const ITACHI_GOKAKYU = {
    id: 0,
    name: 'KATON: GŌKAKYŪ NO JUTSU',
    em: '🔥',
    dmg: '⚔ DAÑO: -40HP · 🔵 COSTO MP: 14',
    efecto: '💀 Quemadura: -2% HP por segundo (4s)',
    buff: '✦ +10% ATK durante 25s',
    dur: '⏱ CD: 13s',
    battle: {
      type: 'katon_gokakyu',
      mpCost: 14,
  ...
  const JutsuSystem = {
    ...
    refreshLibraryForHero() {
      this.activeHeroId = this.getActiveHeroId();
      this.activeLibrary = BASE_JUTSU_LIBRARY.map((jutsu) => ({ ...jutsu }));
      if (this.activeHeroId === 'itachi') {
        this.activeLibrary[0] = { ...ITACHI_GOKAKYU };
      }
    },

    getSkillById(id) {
      return this.activeLibrary[id] || null;
    },
    ...
    getEquippedSkills() {
      return this.state.slots
        .filter((id) => id != null && this.getSkillById(id))
        .map((id) => {
          const skill = this.getSkillById(id);
          return { id, name: skill.name, em: skill.em, battle: skill.battle || null };
        });
    },
    ...
    renderLib() {
      const nextHeroId = this.getActiveHeroId();
      if (nextHeroId !== this.activeHeroId) this.refreshLibraryForHero();
      const lib = this.root.querySelector('#jsuSkillLib');
      if (!lib) return;
      lib.innerHTML = '';

      this.activeLibrary.forEach((jutsu) => {
    ...
    upgradeSkill() {
      ...
      this.syncResources();
      this.setStatus(`⬆ ${this.getSkillById(id)?.name || 'Jutsu'} mejorado → Lv ${this.state.levels[id]}`);
      this.openDetail(id);
      this.renderLib();
      this.renderSlots();
    },
    ...
    equipFromDetail() {
      ...
      if (empty >= 0) {
        ...
        this.setStatus(`⬣ ${this.getSkillById(id)?.name || 'Jutsu'} equipado en slot ${empty + 1}`);
      } else {
    ...
    equipInSlot(slot, id) {
      ...
      this.setStatus(`⬣ ${this.getSkillById(id)?.name || 'Jutsu'} equipado`);
    },
