(function () {
  const missionsData = {
    D: [
      { name: 'Eliminar lobos hambrientos', xp: 4, gold: 8, hp: 138, atk: 4, def: 1, lvl: 1 },
      { name: 'Recuperar suministros robados por goblins', xp: 8, gold: 15, hp: 175, atk: 10, def: 5, lvl: 3 },
      { name: 'Proteger la aldea de jabalíes', xp: 15, gold: 22, hp: 215, atk: 16, def: 9, lvl: 5 },
      { name: 'Investigar ruinas infestadas de ratas gigantes', xp: 23, gold: 35, hp: 258, atk: 22, def: 13, lvl: 7 },
      { name: 'Escoltar a un mercader (bandido)', xp: 35, gold: 50, hp: 305, atk: 28, def: 17, lvl: 9 },
      { name: 'Cazar una bestia nocturna', xp: 45, gold: 75, hp: 385, atk: 37, def: 23, lvl: 12 }
    ],
    C: [
  ...
  const MissionSystem = {
    ...
    startFight(index, rank, btnEl) {
      ...
      if (rank === 'D' && window.BattleRunnerSystem) {
        this.openBattleView(mission, rank, index);
        return;
      }
    ...
    resetToMain() {
      ...
      const battle = this.root?.querySelector('#ms-view-battle');
