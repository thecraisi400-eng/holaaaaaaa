(function () {
  ...
  const ITACHI_JUTSU_OVERRIDES = {
    0: { id: 0, name: 'KATON: GŌKAKYŪ NO JUTSU', em: '🔥', dmg: '-40HP', efecto: 'Quemadura al enemigo -2%HP', buff: 'Aumenta Ataque del personaje en 15%', dur: '13s' },
    1: { id: 1, name: 'KAGE BUNSHIN NO JUTSU', em: '👥', dmg: '0HP', efecto: 'Crea 2 Clones con 20% de Estadisticas', buff: '-', dur: '10s' },
    2: { id: 2, name: 'SHURIKEN JUTSU', em: '🌟', dmg: '-30HP', efecto: 'Crea 5 Shuriken', buff: '-', dur: '15s' }
  };
  ...
  const JutsuSystem = {
    ...
    mount() {
      ...
      this.refreshLibraryForCharacter();
      if (!this.onHeroUpdated) {
        this.onHeroUpdated = () => {
          if (!this.isMounted()) return;
          this.refreshLibraryForCharacter();
          this.renderLib();
        ...
        window.addEventListener('ngs:hero-stats-updated', this.onHeroUpdated);
      }
      this.bindEvents();
      this.renderLib();
      this.renderSlots();
    ...
    unmount() {
      ...
      this.selected = null;
      if (this.onHeroUpdated) {
        window.removeEventListener('ngs:hero-stats-updated', this.onHeroUpdated);
        this.onHeroUpdated = null;
      }
    },
    ...
    getLibrary() {
      const selectedCharacter = this.getCurrentCharacterId();
      if (selectedCharacter !== 'itachi') return BASE_JUTSU_LIBRARY;
      return BASE_JUTSU_LIBRARY.map((jutsu) => ITACHI_JUTSU_OVERRIDES[jutsu.id] || jutsu);
    },

    refreshLibraryForCharacter() {
      const library = this.getLibrary();
      this.state.levels = Array(library.length).fill(1).map((_, idx) => this.state.levels[idx] || 1);
      this.state.slots = this.state.slots.map((slotId) => (library[slotId] ? slotId : null));
    },
    ...
    getEquippedSkills() {
      ...
      return this.state.slots
        ...
        .map((id) => ({ id, name: library[id].name, em: library[id].em }));
    },
    ...
    upgradeSkill() {
      ...
      this.syncResources();
      this.setStatus(`⬆ ${this.getLibrary()[id].name} mejorado → Lv ${this.state.levels[id]}`);
      this.openDetail(id);
      this.renderLib();
      this.renderSlots();
    },
    ...
    equipFromDetail() {
      ...
      if (empty >= 0) {
        ...
        this.setStatus(`⬣ ${this.getLibrary()[id].name} equipado en slot ${empty + 1}`);
      } else {
    ...
    equipInSlot(slot, id) {
      ...
      this.setStatus(`⬣ ${this.getLibrary()[id].name} equipado`);
    },
