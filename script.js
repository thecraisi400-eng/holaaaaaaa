(() => {
  ...
  function openSection(sectionKey, buttonEl) {
    ...
    if (sectionKey === 'batallas') {
      stopHeroPassiveRegen();
      refs.overlay.classList.remove('visible');
      if (window.BatallasNinjaModule?.renderBatallasSection) {
        cleanupCenter();
        window.BatallasNinjaModule.renderBatallasSection(refs.center);
      } else {
  ...
  function mountStartMenu(onChooseCharacter, onLoadSavedGame) {
    ...
    style.textContent = `
      ...
      #ninja-start-root #game{width:355px;height:780px;background:#fff;border-radius:4px;overflow:hidden;position:relative}
      #ninja-start-root .screen{width:355px;height:780px;background:#0d1117;position:absolute;top:0;left:0;display:none;flex-direction:column;overflow:hidden}
      ...
      #ninja-start-root .logo-title{font-family:'Cinzel',serif;font-size:38px;font-weight:900;color:#f0c040;letter-spacing:4px;text-shadow:0 0 20px rgba(240,192,64,.5);text-transform:uppercase}
      #ninja-start-root .logo-sub{font-size:9px;color:#6a82a0;letter-spacing:5px;text-transform:uppercase;margin-top:4px}
      #ninja-start-root .divider-line{width:160px;height:1px;background:linear-gradient(90deg,transparent,#e8a020,transparent);margin:30px 0}
      ...
      #ninja-start-root .menu-btn[data-action="new-game"]{
  margin-bottom:22px;
}
