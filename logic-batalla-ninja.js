(function () {
    ...
    function initNinjas() {
        ...
        const rankTitles = ['Genin', 'Chunin', 'Jonin', 'Anbu', 'Kage'];
        for (let i = 0; i < PARTICIPANT_COUNT; i += 1) {
            ...
            state.ninjas.push({
                ...
                level,
                formulaIdx,
                rankTitle: rankTitles[Math.floor(Math.random() * rankTitles.length)],
                stats
            });
        }
    }
    ...
    function registerCombat(attacker, defender, attackerWon, oldRankAtt, oldRankDef) {
        ...
        state.rankingLogs.push(entry);
        if (state.rankingLogs.length > 40) state.rankingLogs.shift();
        if (defender.isPlayer) {
            state.personalLogs.push(entry);
            if (state.personalLogs.length > 40) state.personalLogs.shift();
        }
    }

    function renderLogLines(entries, emptyText) {
        if (!entries.length) return `<div class="bn-log-line">${escapeHtml(emptyText)}</div>`;
        return entries.slice(-8).map((log) => `
        ...
        return `
            <div style="height:100%; overflow-y:auto; background:#1a0f0a; border-radius:20px; padding:10px;">
                ...
                ${state.personalLogs.length
                    ? state.personalLogs.slice(-20).map((log) => `<div style="background:#2a1a15; margin:4px 0; padding:6px; border-radius:12px;"><span class="${log.colorClass}">${escapeHtml(log.text)}</span></div>`).join('')
    ...
    function render() {
        ...
        if (state.currentView === 'scroll') {
            container.innerHTML = `
                <div class="bn-shell">
                    <button type="button" class="bn-close-top" onclick="window.batallaNinjaSystem.close()">✕ Cerrar</button>
                    <div class="bn-scroll-screen">
                        ...
                        <div class="bn-scroll-note">Abre el pergamino BATALLA NINJA para mostrar la interfaz completa del ranking sin perder el contexto actual del cuadro central.</div>
                    </div>
                </div>
            `;
            return;
                ...
                <div id="bnGameContainer" class="bn-game-container rango-bajo">
                    <div class="bn-inner">
                        <div class="bn-header">
                            ...
                            <button type="button" class="bn-icon-btn" onclick="window.batallaNinjaSystem.togglePersonalLogs()">💬</button>
    ...
    function startTimerLoop() {
        ...
        state.countdownInterval = window.setInterval(updateTimerDisplay, 1000);
    }
